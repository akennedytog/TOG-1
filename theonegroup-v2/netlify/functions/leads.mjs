...
export async function handler(event) {
  ...
  const hubspotEnabled = !!process.env.HUBSPOT_ACCESS_TOKEN;
  if (!hubspotEnabled) {
    console.log('HUBSPOT_ACCESS_TOKEN not configured — running in email-only mode');
  }
  ...
  return json(200, { success: true, hubspot: { enabled: hubspotEnabled, synced: hubspotSynced }, notification: { sent: true }, confirmation: { sent: confirmationSent } });
}
