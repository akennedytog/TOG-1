[Skip to main content](https://developers.figma.com/docs/rest-api/authentication/#__docusaurus_skipToContent_fallback)
...
- REST API
  ...
  - [Figma files](https://developers.figma.com/docs/rest-api/authentication/#)

  - [Comments](https://developers.figma.com/docs/rest-api/authentication/#)

  - [Users](https://developers.figma.com/docs/rest-api/authentication/#)

  - [Version history](https://developers.figma.com/docs/rest-api/authentication/#)

  - [Projects](https://developers.figma.com/docs/rest-api/authentication/#)

  - [Components](https://developers.figma.com/docs/rest-api/authentication/#)

  - [Webhooks](https://developers.figma.com/docs/rest-api/authentication/#)

  - [Activity logs](https://developers.figma.com/docs/rest-api/authentication/#)

  - [Discovery](https://developers.figma.com/docs/rest-api/authentication/#)

  - [Payments](https://developers.figma.com/docs/rest-api/authentication/#)

  - [Variables](https://developers.figma.com/docs/rest-api/authentication/#)

  - [Dev Resources](https://developers.figma.com/docs/rest-api/authentication/#)

  - [Library Analytics](https://developers.figma.com/docs/rest-api/authentication/#)

  - [Errors](https://developers.figma.com/docs/rest-api/errors/)
  - [SCIM API Reference](https://developers.figma.com/docs/rest-api/authentication/#)
...
When you make requests to endpoints in the Figma REST API, you must authenticate the requests you are sending. You can authenticate requests to the Figma REST API in two ways:

- **Recommended:** Create an OAuth app to use OAuth 2, which lets you make requests with access tokens that you obtain on behalf of the users of your app. The advantage of an OAuth app is that you can make requests on behalf of other users, and the requests themselves aren’t made using your personal Figma account. Additionally, if for some reason you leave a company or team, OAuth apps are able to be reassigned by team or organization admins.
...
**Note:** If you’re working with the [Embed API](https://developers.figma.com/docs/embeds/embed-api/), you’ll also need to create an OAuth app even if you won’t be authenticating users.

- Generate a personal access token for your Figma account. A personal access token allows you to make REST API requests using your personal Figma account.


For both OAuth apps and personal access tokens, you’ll specify what [scopes](https://developers.figma.com/docs/rest-api/scopes/) you want to permit. Scopes define what resources the OAuth 2 access token or personal access token. For example, to read the content of files, you would grant the `file_content:read` scope to the OAuth app or personal access token.

## OAuth apps [​](https://developers.figma.com/docs/rest-api/authentication/\#oauth-apps "Direct link to OAuth apps")
...
1. [Create and configure an OAuth app](https://developers.figma.com/docs/rest-api/authentication/#create-an-oauth-app)
2. [Authenticate users](https://developers.figma.com/docs/rest-api/authentication/#authenticate-users)
3. [Use the token to make requests](https://developers.figma.com/docs/rest-api/authentication/#use-oauth-token)
4. [Refresh access tokens](https://developers.figma.com/docs/rest-api/authentication/#refreshing-tokens)
...
**Important:** We recently announced some updates for Figma's developer platform that impact requirements for registering OAuth apps. If you previously created an OAuth app, you will need to take some steps to update and publish it. To learn more, see [Re-publish OAuth Apps](https://developers.figma.com/docs/rest-api/republish-oauth-apps/).

### Create an OAuth App [​](https://developers.figma.com/docs/rest-api/authentication/\#create-an-oauth-app "Direct link to Create an OAuth App")
...
You’ll select whether your OAuth app is private (default) or public when you [configure it](https://developers.figma.com/docs/rest-api/authentication/#configure-an-oauth-app).

### Configure an OAuth app [​](https://developers.figma.com/docs/rest-api/authentication/\#configure-an-oauth-app "Direct link to Configure an OAuth app")
...
01. In the list of your OAuth apps at [figma.com/developers/apps](https://www.figma.com/developers/apps), click the OAuth app that’s in a draft state. If you’ve just created your OAuth app, the configuration modal will already be open.
...
02. On the **General** page you can change the name you entered when you created the app and upload a logo.

03. On the **OAuth credentials** page, click **Add a redirect URL** and add one or more redirect URLs for your OAuth app.
...
04. On the **OAuth scopes** page, select one or more scopes that you want to use for your OAuth app. For example, if your application needs to read the layers and content of a Figma file, select the `file_content:read` scope under **Files**.
...
05. On the **Publish** page, select whether you want your app to be public or private.
...
06. On the **Describe your app** page, add the required logo and description for your OAuth app.

07. On the **Review scopes** page, review the scopes that you selected for your OAuth app on the **OAuth scopes** page.

08. For private OAuth apps, click the **Publish** button in the lower-right corner of the configuration modal. You’re finished! Your private OAuth app is published and available use in your team or organization.

    For public OAuth apps, continue to the next step.

09. On the **Testing instructions** page (public OAuth apps only):
...
### Example: Public OAuth app submission [​](https://developers.figma.com/docs/rest-api/authentication/\#example-public-oauth-app-submission "Direct link to Example: Public OAuth app submission")
...
- **Testing steps**:
1. Visit [https://taskbridge.io](https://taskbridge.io/) and create a free account
...
### Authenticate users [​](https://developers.figma.com/docs/rest-api/authentication/\#authenticate-users "Direct link to Authenticate users")
...
| scope | A space-separated or comma-separated list of [scopes](https://developers.figma.com/docs/rest-api/scopes/) that you need for your application. Scopes determine which endpoints can be accessed. For example, if your application needs to read files and post comments, your scope value would be file\_content:read,file\_comments:write.<br>You can provide the `scope` query parameter a subset of the scopes you selected when you configured your OAuth app. |
...
![An OAuth app authorizon page that requests access to read email, file contents, and other scopes. A blue allow access button and a switch accounts link are at the bottom.](https://developers.figma.com/img/rest-api/ExampleAppOAuth.png)
...
### Use the OAuth 2 Token [​](https://developers.figma.com/docs/rest-api/authentication/\#use-oauth-token "Direct link to Use the OAuth 2 Token")

Once you have obtained the user access token, using it is very similar to the way you use [personal access tokens](https://developers.figma.com/docs/rest-api/authentication/#access-tokens).
...
### Refreshing tokens [​](https://developers.figma.com/docs/rest-api/authentication/\#refreshing-tokens "Direct link to Refreshing tokens")
...
## Personal access tokens [​](https://developers.figma.com/docs/rest-api/authentication/\#access-tokens "Direct link to Personal access tokens")
...
### Generate a personal access token [​](https://developers.figma.com/docs/rest-api/authentication/\#generate-a-personal-access-token "Direct link to Generate a personal access token")
...
5. Set the expiration and [scopes](https://developers.figma.com/docs/rest-api/scopes/) for the token. The scopes will determine which endpoints the token has access to.
...
### Use the personal access token [​](https://developers.figma.com/docs/rest-api/authentication/\#use-the-personal-access-token "Direct link to Use the personal access token")
...
[Previous\\
\\
Introduction](https://developers.figma.com/docs/rest-api/) [Next\\
\\
Scopes](https://developers.figma.com/docs/rest-api/scopes/)

- [OAuth apps](https://developers.figma.com/docs/rest-api/authentication/#oauth-apps)
  - [Create an OAuth App](https://developers.figma.com/docs/rest-api/authentication/#create-an-oauth-app)
  - [Configure an OAuth app](https://developers.figma.com/docs/rest-api/authentication/#configure-an-oauth-app)
  - [Example: Public OAuth app submission](https://developers.figma.com/docs/rest-api/authentication/#example-public-oauth-app-submission)
  - [Authenticate users](https://developers.figma.com/docs/rest-api/authentication/#authenticate-users)
  - [Use the OAuth 2 Token](https://developers.figma.com/docs/rest-api/authentication/#use-oauth-token)
  - [Refreshing tokens](https://developers.figma.com/docs/rest-api/authentication/#refreshing-tokens)
- [Personal access tokens](https://developers.figma.com/docs/rest-api/authentication/#access-tokens)
  - [Generate a personal access token](https://developers.figma.com/docs/rest-api/authentication/#generate-a-personal-access-token)
  - [Use the personal access token](https://developers.figma.com/docs/rest-api/authentication/#use-the-personal-access-token)
